...
|---|---|---|---|
| p1 FHSS (BW250) | 197 B | 1,563 bits ≈ 21–35 records | 1 frame, 545 bits spare |
| p2 DTS (BW500) | 237 B | 1,883 bits ≈ 25–43 records | 1 frame, 865 bits spare |
...
| 6 | GZOOM | u8 code z for the signed ground-motion parameter of §4.4: u = (z − 128) / 4096 px⁻¹ (z = 128 → 0; ±127 codes = ±0.031 px⁻¹, about ±5 m of travel at 130° with h = 1.8 m and ±30 m at 39°; one code ≈ 4 cm / 23 cm), cumulative since the epoch start, not a delta; the renderer applies r′ = r / (1 − u·r) with x scaled by r′/r about the vanishing point, so reversing (u < 0) contracts the ground; a value outside the field starts a new epoch | 18 |
| 7 | GAIN | 3 × 5-bit per-channel code g: gain = 2^((g − 16) / 32) (neutral 16; 2.2 % per code; 0 → ×0.71, 31 → ×1.38), absolute since the epoch start, applied to each 8-bit channel of the base colour and clipped to 0–255 | 25 |
| 8 | HOLE | parent id7, slot2 (0–3 per parent), del1, centre 13 (4 px), r3 ((v+1)·4 px). A HOLE for an existing slot replaces it; del1 = 1 removes it (centre and r ignored). Holes have no identity beyond their slot: they live and die with the parent and are part of its state-hash, so CONFIRM and DIGEST cover them | 36 |
| 9–15 | reserved | Reject frame | — |

**FILL** = `m1 | (pal4 if m = 0, else RGB444 12) | g1 | [dir3 (22.5° steps over 180°) | dl3]`. `dl3` is a two's-complement code c ∈ −4..+3 giving ΔL = 16·c in 8-bit units (−64..+48): the colour at the shape's bounding-box extreme in direction `dir3` is the base colour + ΔL on every channel and at the opposite extreme base − ΔL, linear between, each channel clipped to 0–255 (the RGB444 base is scaled ×17 to 8 bits first).
...
- **Grid to pixels:** px = gx·8, py = gy·8 on the 8 px grid (W × H = 48 × 32 cells) and px = gx·4, py = gy·4 on the 4 px grid (96 × 64 cells); W and H are the grid's cell counts, never the 12 × 8 tile grid of the `TileDeltaFrame` header.
...
- u **adds** across frames (r′′ = r / (1 − (u1 + u2)·r)), so the cumulative signed u8 of §3.3 is idempotent under loss.
...
| **V3 dead air** | no VS frame for 10 s | 20 B beacon: the epoch's absolute anchor (HZN ABS, 58 bits, or NO_HORIZON) + STATUS (+ ANOM), so a receiver that missed the epoch start or restarted re-anchors from any beacon | — | — | nothing else | "NO VECTOR DATA" |

- **How the level reaches the tractor: one mapping for the quality byte.** The existing `0x63` command carries mode 9 and a quality byte 1–100 with a single meaning: the byte is the **detail ceiling**. Its band selects the level — 60–100 → V0, 40–59 → V1, 20–39 → V2, 1–19 → V3 — and the value inside the band scales the INSERT/L4 budget and the residual threshold linearly, so the operator's slider (§6) chooses detail within V0 and the ladder chooses the band. Automatic degradation sends `min(operator value, band ceiling)` — 59 for V1, 39 for V2, 19 for V3 — the base remembers the operator's value and restores it as the ladder climbs, and the ack matching of §6 compares the byte actually sent. The tractor's self-selected level (D-VS6b) can only lower the effective band; the LL field reports the effective level either way. Both ends clamp the quality byte to 20–100 today — the tractor in `_apply_encode_mode` (`x8/camera_service.py:1240-1253`) and the base in its ack matcher (`TRACTOR_QUALITY_MIN = 20` in `_ack_matches_body`, `bs/image_rx_daemon.py:1003-1047`), which must clamp identically or an ack never matches; for mode 9 both clamps become 1–100, so the V3 band exists on the wire and its ack matches on the first attempt. In vector mode the settings slider covers 60–100 only (`vector_detail`, §6; the tile-mode slider keeps its 20 floor, `settings.html:216`), so the operator chooses detail within V0 and never a band: V1–V3 come only from the ladder or the tractor's self-select. In V3 the base repeats the command at the RS-12.14 rate limits.
...
- **Base heartbeat (the silence clock needs it).** On the shipped path base → tractor traffic is event-driven — `image_rx_daemon` only drains queued commands, and its periodic probe is bench-only (`LIFETRAC_REACTIVE_FIRE=0` in production, `bs/image_rx_daemon.py:354-359`) — so a healthy idle base is silent indefinitely, and silence alone would walk a healthy tractor down the ladder. VECTOR mode therefore adds `LINK_HB` (`0x71`, §4.6.3): whenever no command other than a heartbeat copy has left the gate for `T_hb` = 5 s, measured from the *first* copy of the previous heartbeat, `image_rx_daemon` sends a 12 B heartbeat carrying the rung and the number of VS frames it decoded since the last one, two copies 1.0 s apart on FHSS (every base copy rides the shared command gate, §4.6.3; the second copy neither resets the silence clock nor delays the next heartbeat, so the cadence is a true 5 s). Cost: 2 × 20.6 ms per 5 s at F0 (0.8 % of airtime), 2 × 72.2 ms at F2 (2.9 %). The tractor's silence clock counts commands and heartbeats alike: **one level down per 20 s of silence** (four heartbeats missed), never without the heartbeat in service, and the `frames_rx` field tells the tractor whether its own frames are getting through, which keeps it off the rendezvous path (§4.6.4) while the forward link is alive.
...
| `0x70` | `RUNG_HELLO` | both | `u8 rung, u8 req_seq` (the epoch is already in the hop header's `epoch_le`) | new rung, in alternating slots (base: even hop slots, through the gate; tractor: odd slots; on DTS the base at the apply instant + k × 1.0 s and the tractor offset by 500 ms) until the first peer frame is decoded (rule 4); also the beacon and sweep body on the rendezvous rung |
...
On air under schema 1 (8 B hop header + `0xFB` + opcode + args, `pack_command_frame`, `bs/lora_proto.py:1022`): `RUNG_REQ` 12 B, `RUNG_ACK` 14 B, `RUNG_CONF` 17 B on FHSS / 14 B on DTS, `RUNG_HELLO` 12 B, `LINK_HB` 12 B. At F2 a 12 B frame is 72.2 ms and 14–17 B are 82.4 ms; at F0, 20.6 and 23.2–25.7 ms (computed with `lora_time_on_air_ms`). Schema 2 adds its rung byte and MIC to each.
...
2. A healthy tractor still beacons once per 10 s on the rendezvous channel (§4.6.2 item 4), and the beacon's `rung` field names the tractor's **working** rung, so a base that lost the working rung and parks there learns within 10 s where the tractor is, without a scan.
3. What the listener does with a beacon depends on the rung it names. A beacon from a healthy peer names a working rung: the listener retunes to that rung directly (the peer never left it) and the lease of §4.6.3 takes over from the first frame it decodes there. A beacon that names the rendezvous rung comes from a peer that is itself deaf: both stay on the rendezvous rung and negotiate upward with the normal flow, one rung at a time, subject to the policy hysteresis.
...
| Base | `bs/image_rx_daemon.py` | `_CODEC_NAMES[6] = "vector"` (`:223`); accept mode 9 in `_on_encode_mode_msg` (`:1175-1219`; 8 landed in #131); send `LINK_HB` after `T_hb` = 5 s of command silence, through the same gate as every command (§4.5.4); make the ack matcher's quality clamp mode-aware (`_ack_matches_body`, `:1032-1047`: 1–100 when the commanded mode is 9, 20–100 otherwise), or every V3 ack mismatches and the base retries until cool-down |
...
| Base | `bs/web/settings.html` | "vector" in the encode-mode selector; in that mode the quality slider becomes the `vector_detail` control (label "detail", range 60–100, default 80) and shows the band the ladder currently clamps it to (§4.5.4); V3 is never operator-selectable |
...
5. **Old clients.** Cache-busted script tags (`index.html:436-444`) only help a page that reloads. A browser that connected before the deploy has no vector renderer at all, so the server fails it closed without needing anything from it: clients that know the vector schema connect to `/ws/state?schema=2` (the query string is available at accept time in `_admit_ws`; the socket itself only sends snapshots, `bs/web_ui.py:1648-1660`). To a connection without it, web_ui sends snapshots with no `vector_scene` and, while VECTOR is active, a full set of opaque-black tiles (all 96 cells, one cached 32×32 black WebP blob) in place of the photo, plus the tile ages for its staleness overlay. The shipped `blitTiles` paints whatever tiles it is given and never clears on its own (`bs/web/img/canvas_renderer.js:74-88`), so an empty list would have left the stale photograph in place; the black set is what old code actually paints. An old page therefore shows a black canvas with an age, never a stale photo. A new `vector_renderer.js` that cannot parse a newer snapshot shows "VECTOR?" rather than nothing.
...
- The injector (`feed_canvas.py`) refuses to run when `image_rx_daemon` has published a frame in the last 60 s or a control session is active; the read-only Lab encoder refuses only while a control session is active, because encoding the `live` canvas while frames arrive is exactly what the `live` source is for. `LIFETRAC_VECTOR_LAB_BENCH=1` overrides both gates. Every use is audit-logged.
...
| File | What it covers |
|---|---|
| `test_vector_codec.py` | Golden vectors for every record, including all four LAYER_CLEAR ranges, the DIGEST/CONFIRM CRCs (`2A 34 12 78 56` + the neutral render state → 0xD2) and the eight palette slots; byte0 ≤ 0x7F; frames ≤ F for F ∈ {197, 237}; EG2 round-trip; Kraft completeness; **trailing padding of 0–16 bits at every leftover width**; key frame = absolute HZN + LAYER_CLEAR (ABS 70, NO_HORIZON 52) |
...
| 4 | Encoder repeat-once of the epoch start | The absolute HZN + LAYER_CLEAR re-sent in the next frame (idempotent per epoch and range, §3.3); the daemon's own duplicate fires only on local TX failures, never on air loss | 70 bits per epoch (52 with a NO_HORIZON anchor) |
...
| 26 | Coordinated modem-rung switch (D-VS8, §4.6) | +2.5 dB per SF step at the range edge; VS1 keeps whole-scene coverage at SF9 (35 B bodies), where a tile frame carries at most one minimal `mono_g4` tile; rendezvous beacon (12 B on air) costs 0.7 % of airtime at F2 | L072 CFG key, scheduled apply, five `0xFB` opcodes, SIL model, two bench legs |
