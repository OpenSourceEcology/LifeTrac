...
| 2 | DIGEST | n_live7, crc8 over the live shapes sorted by id ascending, each serialised as `id u8, define-hash u16le, n_inserts u8, state-tag u8` (the CONFIRM tag2, 0–3). CRC-8 is poly 0x07, init 0x00, no reflection, xorout 0x00 (check value of "123456789" = 0xF4); golden vector: one shape `2A 34 12 03 01` → 0x04. define-hash is CRC-16/CCITT-FALSE (poly 0x1021, init 0xFFFF, no reflection) over the define record's packed bytes, prefix code through FILL, zero-padded to a byte | 25 |
...
| 6 | GZOOM | u8 code z: ground scale s = 2^(z / 128) about the ground anchor (z = 0 → 1.0, 128 → 2.0, 255 → 3.97; 0.54 % per code), absolute since the epoch start, not a delta (§4.4); a scale beyond code 255 starts a new epoch | 18 |
...
| 8 | HOLE | parent id7, slot2 (0–3 per parent), del1, centre 13 (4 px), r3 ((v+1)·4 px). A HOLE for an existing slot replaces it; del1 = 1 removes it (centre and r ignored). Holes have no identity beyond their slot: they live and die with the parent and change its state-tag, so CONFIRM and DIGEST cover them | 36 |
...
- **CONFIRM tag.**
  - tag2 = the low 2 bits of crc8 (the DIGEST variant, §3.3) over `define-hash u16le, local offset dx s8, dy s8 (grid cells), base colour RGB444 u16le, n_inserts u8`, computed from the tractor's mirror.
...
5. **Old clients.** Cache-busted script tags (`index.html:436-444`) only help a page that reloads. A browser that connected before the deploy has no vector renderer at all, so the server fails it closed without needing anything from it: clients that know the vector schema connect to `/ws/state?schema=2` (the query string is available at accept time in `_admit_ws`; the socket itself only sends snapshots, `bs/web_ui.py:1648-1660`). To a connection without it, web_ui sends snapshots with `tiles=[]` and no `vector_scene` while VECTOR is active — the shipped bundle already blanks the canvas on an empty tile list (`bs/web_ui.py:1433`) and its staleness overlay shows the age — so an old page shows a black canvas with an age, never a stale photo. A new `vector_renderer.js` that cannot parse a newer snapshot shows "VECTOR?" rather than nothing.
...
| File | What it covers |
|---|---|
| `test_vector_codec.py` | Golden vectors for every record, including all four LAYER_CLEAR ranges, the DIGEST/CONFIRM CRCs (`2A 34 12 03 01` → 0x04) and the eight palette slots; byte0 ≤ 0x7F; frames ≤ F for F ∈ {197, 237}; EG2 round-trip; Kraft completeness; **trailing padding of 0–16 bits at every leftover width**; key frame = absolute HZN + LAYER_CLEAR (ABS 70, NO_HORIZON 52) |
...
| 4 | Encoder repeat-once of the epoch start | The absolute HZN + LAYER_CLEAR re-sent in the next frame; the daemon's own duplicate fires only on local TX failures, never on air loss | 70 bits per epoch (52 with a NO_HORIZON anchor) |
