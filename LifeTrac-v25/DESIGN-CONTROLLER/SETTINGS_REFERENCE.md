...
| LIFETRAC_PENDING_RETRY_MAX_GAP_S | `8.0` | float s | `base_station/image_rx_daemon.py` | live | RS-12.14: cap on the backoff gap. |
...
| LIFETRAC_CMD_STREAM_MIN_GAP_S | `1.0` | float s ≥ 0.12; `0.12` = pre-RS-12.14 | `base_station/image_rx_daemon.py`, rule `cmd_timing.pump_min_gap`; harness `-CmdStreamMinGapS` | live | RS-12.14: minimum spacing between ANY two base command sends while fragments are flowing (a stream is active for IDLE_DRAIN_QUIET_S after the last fragment). Bounds the base's self-deafening and the follower disruption regardless of who queued the command. |
