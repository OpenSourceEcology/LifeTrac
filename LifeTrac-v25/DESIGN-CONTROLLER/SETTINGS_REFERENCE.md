...
| LIFETRAC_TILE_STALE_AFTER_MS | `20000` | int ms; **must exceed the encoder sweep rotation** `n_tiles/(SWEEP_STEP × fps)` | `base_station/web_ui.py:1073` | live | Staleness horizon for the 0x6C report. 30000 at the 2 fps bench point (24 s rotation); a low value false-positives on static tiles every rotation. |
