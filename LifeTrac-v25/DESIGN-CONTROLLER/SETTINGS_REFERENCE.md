...
| host_cfg_profile power clamp | tier − max(0, gain−6), floored by hw ceiling | 0 or [2,30] dBm | `firmware/murata_l072/host/host_cfg_profile.c:64` | live | The ERP clamp. Result is only tested for ==0 at `:103`; it is **never programmed into the PA**. |
| Power-clamp result → RegPaConfig | none — `activate()` never calls `sx1276_set_tx_power_dbm` | n/a | `firmware/murata_l072/host/host_cfg_profile.c:184` | **dead** | Gap: the §A5 ERP clamp gates profile acceptance but does not set power. |
...
HostLink framing, the 23-key CFG table, request/URC opcodes, and the H7-side mirror of the same wire contract.
...
| Setting | Default | Range | Where | Status | Purpose |
| --- | --- | --- | --- | --- | --- |
| CFG_KEY_COUNT | `23U` | `_Static_assert` against the table size (`host_cfg.c:144`) | `firmware/murata_l072/include/host_cfg_keys.h:40` | live | Descriptor table size. Keys 0x0F and 0x12 are intentionally unallocated. |
...
| CFG_KEY_TX_POWER_DBM (0x01) | `14U` | normalised (not rejected) into [2,17] at `host_cfg.c:189`, clipped again in the radio | `firmware/murata_l072/host/host_cfg.c:24` | live | The only setting that actually programs RegPaConfig. **Not** cross-checked against the profile ERP clamp — a host can set 17 dBm regardless of declared antenna gain. `cfg_init()` seeds the table but never calls the apply fn, so this default never reaches the radio; the 14 dBm at boot comes from `sx1276_init()`. |
...
| --- | --- | --- |
| Power-clamp result → RegPaConfig | `firmware/murata_l072/host/host_cfg_profile.c:184` | **The ERP clamp never programs the PA.** It only decides whether a profile is accepted. Declaring 12 dBi of antenna gain does not lower conducted power — CFG key 0x01 alone does. |
...
| FHSS seed (farm_id, node_id, epoch) | `(0, 0, 0)` | `firmware/murata_l072/host/host_cfg_profile.c:194` | Hardcoded at the only production call site; no CFG key reaches the arguments. | **Highest-value item here:** every node in the fleet currently derives the identical hop permutation. Needs three new CFG keys plus a re-init path. |
...
| Conducted TX power derived from antenna gain | not applied | `firmware/murata_l072/host/host_cfg_profile.c:184` | The ERP clamp computes a number and throws it away. | Call `sx1276_set_tx_power_dbm(clamped)` in `host_cfg_profile_activate()`. |
