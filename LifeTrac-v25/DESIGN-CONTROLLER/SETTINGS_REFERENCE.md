...
| fragment.py re-exports | magic = TELEMETRY_FRAGMENT_MAGIC; cap = IMAGE_FRAG_AIR_CAP_MS; default profile BW250 | frag_seq rolls 0..255 | `firmware/tractor_x8/x8_image_pipeline/fragment.py:43` | live | Thin re-export so tractor callers do not reach into the base tree. |
...
| AutoRadioPolicy.DEAD_AIR_S | `10.0` | unconstrained | `base_station/web_ui.py` | live | RS-12.16 (2026-09-15): no fragment for this long AFTER the link was streaming counts as unhealthy — the FHSS lock-loss signature (healthy links never gap > ~1 s; lock losses ran 20–53 s). |
| AutoRadioPolicy.STREAM_MEMORY_S | `60.0` | unconstrained | `base_station/web_ui.py` | live | RS-12.16: silence longer than this is an IDLE tractor, not a sick link — dead-air stops counting, so an absent tractor cannot flap the profile. A counter that never advanced never starts the clock. |
| AutoRadioPolicy.LOSS_RATE_MAX | `0.25` | 0..1 | `base_station/web_ui.py` | live | RS-12.16: fragments missing / expected over the evaluation window above which p2 degrades to p1. Bench floor is 1–8 %; the broken old firmware ran 39 %. Re-tune in RS-1.4. |
| AutoRadioPolicy.LOSS_MIN_FRAGS | `20` | ≥ 1 | `base_station/web_ui.py` | live | RS-12.16: the loss window must hold this many EXPECTED fragments before a rate is judged; smaller windows pass `loss_rate=None` (no opinion). |
| AutoRadioPolicy.RESYNC_AFTER_S | `20.0` | > 12 (phase-A ACK window) | `base_station/web_ui.py` | live | RS-12.18 (leg V, 2026-09-15): the daemon reverts a switch on its own (no tractor ACK in 12 s, or no frames on the new profile in 45 s) and publishes its actual profile in every link_stats sample; a disagreement with the policy's belief that persists this long is adopted as a revert (fresh min-gap + dwell). Shorter than the handshake would mistake a switch in flight for a revert. |
...
| HOST_TYPE_STATS_DUMP_REQ | `0x41` | n/a | `firmware/murata_l072/include/host_types.h:40` | live | Emits STATS_URC 0xC1 with the counter block: 132 B originally, additive tails since — RS-11.5 → 144 (tx_fifo_rb_ok/bad, tx_done_early), RS-12 → 152 (rx_urc_lost, rx_pretx_drained), RS-12.10 → 168 (rx_fifo_skip, tx_deaf_max_us, tx_deaf_sum_us, tx_done_to_rearm_max_us), RS-12.15 → 208 (the six-entry `fhss_dec_*` consider_remote decision histogram, clk_demotion_reset, clk_demotion_kept, tx_first_anchor, tx_stream_streak_max). Parsers are offset-guarded; `check-stats-layout` pins the producer. |
...
| HOST_TYPE_STATS_URC | `0xC1` | HOST_STATS_PAYLOAD_LEN = 208 (RS-12.15) | `firmware/murata_l072/include/host_types.h:58` | live | 52 u32 LE fields, additive-only (132 B / 33 fields originally — see the STATS_DUMP_REQ row for the tail history). |
...
| HOST_STATS_PAYLOAD_LEN | `208U` (RS-12.15; was 132U) | `_Static_assert` pins `sizeof(host_stats_wire_t)`; `tools/check_mh_wire_sync.py` pins the H7 mirror | `firmware/murata_l072/include/host_types.h:220` | live | Serialized STATS_URC payload size. |
...
| ipc_to_h747 IpcWriter | device `/dev/ttymxc1`; FRAME_MARKER 0xA5; FLAG_KEYFRAME 0x01, MOTION 0x02, WIREFRAME 0x04 | n/a | `firmware/tractor_x8/x8_image_pipeline/ipc_to_h747.py:33` | live | Length-framed X8→H747 image transport with CRC8-SMBUS; skipped entirely in LoRa-bridge mode. |
...
| Setting | Default | Range | Where | Status | Purpose |
| --- | --- | --- | --- | --- | --- |
| LIFETRAC_NO_PARK_LAST | `"0"` | `"1"` enables | `firmware/tractor_x8/image_tx_daemon.py` (`NO_PARK_LAST`) | live | RS-12 strict hold: the final fragment of a train is submitted only after the penultimate's TX_DONE plus NO_PARK_LAST_GAP_MS, instead of riding the firmware's fire-on-TX_DONE ~42 ms behind it. Removes the penultimate-of-N lock on long trains (3.2 % → 1.5 % synth); costs ~+38 ms per held train. |
| LIFETRAC_NO_PARK_LAST_GAP_MS | `80` | int ms 0..5000 | `firmware/tractor_x8/image_tx_daemon.py` | live | Minimum gap between the penultimate's TX_DONE receipt and the final's submission under the hold. |
| LIFETRAC_NO_PARK_LAST_MIN_FRAGS | `3` | int ≥ 1 | `firmware/tractor_x8/image_tx_daemon.py` (`final_hold_applies`) | live | RS-12.13 (2026-09-12): the hold applies only to trains of at least this many fragments, so 1–2-fragment camera trains are never held. |
...
| LIFETRAC_ALIGNED_PUMP | `"1"` (enabled) | "0" disables | `base_station/image_rx_daemon.py:204` | live | Completion-aligned command pump (RS-1.x); "0" falls back to idle-drain-only TX (since RS-12.11 the idle drain itself is gated by LIFETRAC_IDLE_DRAIN_QUIET_S), the behaviour the 171/1 convergence soak measured. |
| LIFETRAC_IDLE_DRAIN_QUIET_S | `1.5` | float s ≥ 0; `0` = pre-RS-12.11 behaviour (A/B control) | `base_station/image_rx_daemon.py` (`IDLE_DRAIN_QUIET_S`), rule in `base_station/cmd_timing.py` | live | RS-12.11 (2026-09-12): the idle-link command drain runs only after this much time with no fragment. It used to fire on every empty 0.25 s poll — 250–300 ms after the last fragment, exactly where the next 2-fragment camera train\'s first fragment lands — and the base\'s own TX deafened it (kill zone ≈ command ToA + 100 ms fragment). In-stream commands ride the completion-aligned pump, which also opens on a train\'s last-index fragment. Gate: camera loss 3.1 % → 1.0 %. |
| LIFETRAC_PENDING_RETRY_BACKOFF | `2.0` | float ≥ 1; `1` = fixed 0.4 s gap (pre-RS-12.14) | `base_station/image_rx_daemon.py`, rule `cmd_timing.pending_retry_gap` | live | RS-12.14 (2026-09-12): exponential backoff between retries of one pending (ack-driven) command: 0.4, 0.8, 1.6, 3.2, 6.4 s … capped by MAX_GAP. On FHSS the acks mostly do not come back, and a fixed 0.4 s retry fired 222 keyframe requests in 300 s (leg I) — each a base TX that skips the follower. |
| LIFETRAC_PENDING_RETRY_MAX_GAP_S | `8.0` | float s ≥ 0.1 (smaller values are clamped to 0.1) | `base_station/image_rx_daemon.py` | live | RS-12.14: cap on the backoff gap. |
| LIFETRAC_PENDING_GIVEUP_COOLDOWN_S | `30.0` | float s; `0` disables | `base_station/image_rx_daemon.py`, rule `cmd_timing.giveup_cooldown_active` | live | RS-12.14: after a pending command gives up (attempts/deadline exhausted), the same opcode is refused for this long — a self-heal re-trigger used to restart the storm immediately. Counted as `cmd_cooldown_drops` on the stats line. |
| LIFETRAC_CMD_STREAM_MIN_GAP_S | `1.0` | float s ≥ 0.12; `0.12` = pre-RS-12.14 | `base_station/image_rx_daemon.py`, rule `cmd_timing.pump_min_gap`; harness `-CmdStreamMinGapS` | live | RS-12.14: minimum spacing between any two base command dispatches while fragments are flowing (a stream is active for IDLE_DRAIN_QUIET_S after the last fragment); 0.12 s when idle. Since the PR #121 review round (2026-09-14) ONE shared gate (`cmd_timing.send_gate_open`) covers every send path — aligned pump, idle drain (one dispatch per idle pass), profile switch and CONF, reactive probe — measured from the previous dispatch's last on-air copy; a multi-copy command sends one copy at once and queues the rest behind the same gate. Closed-gate checks are counted as `cmd_gate_held` (every check, on every path), real deferrals as `cmd_gate_deferred` -- a closed gate when the caller had a send due, on EVERY gate path: pump and idle drain (a pending retry past its gap or a queued legacy entry), profile switch / Phase-A resend / CONF / probe (they only ask when about to send) -- and queued copies as `cmd_copies_deferred` on the stats line (PR #124 review, 2026-09-14). Bounds the base's self-deafening and the follower disruption regardless of who queued the command. |
...
| link_stats.frags_expected / frags_missing | monotonic counters | ≥ 0 | `base_station/image_pipeline/reassemble.py` (ReassemblyStats), published by `image_rx_daemon._link_stats_worker` | live | RS-12.16: `expected` is booked when a frame is finalized (completed or timed out), `missing` only on timeout as total − fragments present (parity-reconstructed fragments count as present). The auto policy takes missing/expected over its own window; `rx_frames_seen` going flat is its dead-air input. |
...
| LIFETRAC_TILE_STALE_ENABLE | `1` | `"1"` = on, anything else off | `base_station/web_ui.py:1072` | live | F10 (2026-08-01): base-side stale-tile scan worker — publishes `0x6C` reports + the `status/tile_age` aggregate. |
| LIFETRAC_TILE_STALE_AFTER_MS | `20000` | int ms; **must exceed the encoder sweep rotation** `n_tiles/(SWEEP_STEP × fps)` | `base_station/web_ui.py:1073` | live | FLOOR of the staleness horizon for the 0x6C report (RS-4.15, 2026-09-12: effective horizon = max(this, ROTATION_FACTOR × the slowest recent tile-refresh interval), capped by LIFETRAC_TILE_STALE_MAX_MS). 30000 at the 2 fps bench point (24 s rotation); a low value false-positives on static tiles every rotation. |
| LIFETRAC_TILE_STALE_PERIOD_S | `3.0` | float s | `base_station/web_ui.py:1075` | live | Scan/publish cadence for stale reports and tile-age telemetry. Level-triggered: a lost report is superseded by the next. |
| LIFETRAC_TILE_STALE_ROTATION_FACTOR | `1.5` | float; `0` = F10 fixed horizon | `base_station/web_ui.py` | live | RS-4.15: multiplies the measured sweep rotation (slowest tile-refresh interval in the window) to form the horizon. A lost tile never refreshes, so it cannot inflate the estimate. 102 → 13 TILE_STALE per 300 s on a moving scene. |
| LIFETRAC_TILE_STALE_MAX_MS | `120000` | int ms | `base_station/web_ui.py` | live | RS-4.15: cap on the effective horizon so a dead link still reports. |
| LIFETRAC_TILE_STALE_ROTATION_WINDOW_MS | `60000` | int ms | `base_station/web_ui.py` | live | RS-4.15: how far back refresh intervals count toward the rotation estimate. |
| LIFETRAC_TILE_STALE_REPEAT_MIN_S | `10.0` | float s | `base_station/web_ui.py` | live | RS-4.15: an identical stale report is not re-sent inside this interval (level-triggered advisory; the tractor is already on it). |
| LIFETRAC_KF_ON_REASM_TIMEOUT | `0` | `"1"` enables | `base_station/image_rx_daemon.py:199` | live | F10: keyframe request on reassembly timeout — measured net harmful (+1.88 pts loss, −16% frames); default OFF, 0x6C covers the legitimate job. |
| LIFETRAC_KF_ON_SEQ_GAP | `0` | `"1"` enables | `base_station/web_ui.py:1095` | live | F11: keyframe request on every base_seq gap — default OFF after the 2026-08-02 on-air A/B (6 gaps, 6 suppressions, zero canvas harm). |
| LIFETRAC_RXCONT_ARM | `1` | 0/1 — **NEVER ship 0** | `firmware/tractor_x8/image_tx_daemon.py:410` | diagnostic | RS-11.5 leg C: 0 = tractor never arms RXCONT (command downlink fully deaf; in-stream delivery lives in the inter-fragment listening gaps this disables). |
| LIFETRAC_TX_POWER_DBM | *(empty)* | int 2..17, anything else warned + ignored | `firmware/tractor_x8/image_tx_daemon.py:613` | diagnostic | RS-11.5: CFG_SET power override at daemon startup; firmware ERP clamp still applies on top. Bench tip: 17 ≈ 1.4× loss reduction vs default 14 pending RS-11.6. |
| RxcontArm (radio-monitor harness) | `1` | `[ValidateSet(0,1)]` | `firmware/x8_lora_bootloader_helper/run_live_radio_monitor.ps1` | diagnostic | Passthrough to LIFETRAC_RXCONT_ARM; recorded in the archive `params.txt`. |
| TxPowerDbm (radio-monitor harness) | `""` | string dBm, empty = profile default | `firmware/x8_lora_bootloader_helper/run_live_radio_monitor.ps1` | diagnostic | Passthrough to LIFETRAC_TX_POWER_DBM; recorded in the archive `params.txt`. |
...
| roi.DEFAULT_ROIS_BY_MODE | loading (1,10,4,7); driving (3,8,0,4); idle (0,11,0,7); reverse (3,8,4,7) | inclusive col/row bounds on the 12x8 grid | `firmware/tractor_x8/x8_image_pipeline/roi.py:21` | live | Per-mode default ROI rectangles. |
| RoiPlanner grid / hint_ttl_ms | grid 12x8, hint_ttl_ms 5000, mode "idle" | n/a | `firmware/tractor_x8/x8_image_pipeline/roi.py:38` | live | An operator-painted CMD_ROI_HINT expires after 5 s. The hint path only arrives over the M7 back-channel, which is disabled in bridge mode. |
...
| tile_cache.DEFAULT_HISTORY | `4` | n/a | `firmware/tractor_x8/x8_image_pipeline/tile_cache.py:32` | live | Constructor default; camera_service always passes an explicit value. |
...
| IdleDrainQuietS | `"1.5"` | string → LIFETRAC_IDLE_DRAIN_QUIET_S on the rx daemon; `"0"` = old behaviour | `firmware/x8_lora_bootloader_helper/run_live_radio_monitor.ps1` | live | RS-12.11 A/B knob; recorded in params.txt as `idle_drain_quiet_s`. |
...
| LIFETRAC_TRACTOR_DETECTOR_WEIGHTS | `firmware/tractor_x8/x8_image_pipeline/detect_nanodet.py:64` | Empty means the stub; but `_NanoDetOnnx.infer()` returns `[]` unconditionally, so **the ONNX branch produces no detections either**. |
| _NanoDetOnnx input_size / score_threshold | `firmware/tractor_x8/x8_image_pipeline/detect_nanodet.py:50` | Not env-settable and not reachable — camera_service never constructs a detector. |
| CLASS_TABLE (tractor) | `firmware/tractor_x8/x8_image_pipeline/detect_nanodet.py:27` | No detections are ever produced, so the class table never reaches the wire. |
...
| capture.py DEFAULT_WIDTH / HEIGHT / FPS | `firmware/tractor_x8/x8_image_pipeline/capture.py:52` | camera_service does not use this module. Note DEFAULT_FPS 10 disagrees with the live LIFETRAC_CAMERA_FPS default of 2.0. |
| CaptureRing depth / join_timeout_s | `firmware/tractor_x8/x8_image_pipeline/capture.py:224`, `:248` | Standalone capture pipeline only. (There is **no** identifier named `FrameRing` anywhere in the tree.) |
| tile_diff.DEFAULT_THRESHOLD / PHASH_SIDE / grid defaults | `firmware/tractor_x8/x8_image_pipeline/tile_diff.py:38`, `:89` | camera_service implements its own diff inline (byte-diff or L1 magnitude) and never imports this module. |
| encode_motion.SEARCH_RANGE | `firmware/tractor_x8/x8_image_pipeline/encode_motion.py:33` | Unreachable — `motion_only` is implemented as a WebP quality cap, not real motion estimation. |
