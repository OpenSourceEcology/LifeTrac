...
- **Rendezvous:** a deaf base parks on the rendezvous channel; a deaf tractor sweeps `RUNG_HELLO` across the hop set (one channel per slot; every 2 s on DTS); a healthy tractor beacons once per 10 s on the rendezvous channel (12 B on air, 0.7 % of airtime at F2); the cold-start scan parks there first.
