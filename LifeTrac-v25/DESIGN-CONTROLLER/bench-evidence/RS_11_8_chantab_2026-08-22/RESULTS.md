...
~7 s ticker — 49/50 channels "hot" (1–7 hits each; per-channel maxima
mostly −44…−58 dBm with three louder outliers: −30 at 907.25, −35 at
911.75, −38 at 910.75), and the survey therefore CANNOT discriminate
channels at the standard 30 s dwell / −75 dBm cut while that ticker is
active. The one zero-hot
...
1. **The ~7 s ticker hits nearly every 30 s dwell wherever the receiver
   sits** (1–7 hits/channel across the whole band — 908.25 caught it
   only once; the RS-11.6 leg-3 "flat −43 on every channel" signature,
   reconfirmed on the x.25/x.75 grid). Its per-channel max tapers toward
   the top edge (−56 at 927.25 vs −44…−50 mid-band), with the three
   outliers noted in the headline sitting well above the ticker's
   typical level.
...
- A single chantab survey cannot seed the hail-set constants. Options,
  ...
  threshold above the ticker's expected 1–7/30 s); (c) longer dwells.
