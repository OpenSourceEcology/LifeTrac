...
> **Scope (2026-04-26):** LoRa-only per [MASTER_PLAN.md](MASTER_PLAN.md). Line items below for **Cat-M1 IoT SIM**, **cellular SMA antenna**, and **second cellular SIM** on the base station are **archived** — do not order. Operator-browser ↔ base-station LAN/WiFi/Ethernet remains in scope.
...
| **Arduino Portenta X8 (ABX00049)** | 1 | $200 | $200 | [Arduino Store](https://store-usa.arduino.cc/products/portenta-x8) | Linux on i.MX 8M Mini quad Cortex-A53 + **STM32H747 co-MCU**. The H747 co-MCU runs the M7+M4 realtime firmware bare-metal; the i.MX 8M Mini runs Yocto Linux for SSH, logging, MIPI camera, and Docker services. Canonical for both tractor and base per [MASTER_PLAN.md §8.9](MASTER_PLAN.md). |
...
| **Arduino Opta WiFi (AFX00002)** | 1 | $150 | $150 | [Arduino Store](https://store-usa.arduino.cc/products/opta-wifi) | Industrial PLC: 4× 10 A relays + 8 digital/analog inputs + RS-485 + Ethernet. Acts as Modbus-RTU **slave**; carries valve coils + watchdog. **WiFi/BLE radios disabled in firmware** — used only for hydraulic-system I/O over Modbus-RTU. "WiFi" SKU is bought for stock availability, not for the radio. |
| **Opta Ext D1608S** expansion (AFX00006) — **8× solid-state relays (SSR), 24 VDC / 2 A** + 16× programmable voltage inputs (digital 0–24 VDC or analog 0–10 V) | 1 | $151 | $151 | [Arduino Store](https://store-usa.arduino.cc/products/opta-ext-d1608s) | Adds the remaining 4 directional-valve coil drivers on **SSR** channels (sub-ms switching, no contact bounce — see MASTER_PLAN §8.18) + 4 spare SSR for aux. 16 inputs for ignition / mode switch / limit switches. **Do not confuse with the D1608E (AFX00005), which is the electromechanical-relay variant.** |
...
- **Portenta X8 on both nodes (canonical):** X8 is the **only** supported compute module on tractor and base for SKU consistency per [MASTER_PLAN.md §8.9](MASTER_PLAN.md). The X8 contains a full STM32H747 as a co-MCU running the M7+M4 valve/radio firmware bare-metal, plus an i.MX 8M Mini running Yocto Linux + Docker for SSH field debug, log rotation, MIPI camera, container-based telemetry recorder, and one Yocto image to maintain across both nodes. If the X8 is unavailable at procurement time, treat it as a §7 open decision — do not silently substitute.
...
| ``hydraulic.spool_type`` | Tier 1 / **Parker D1VW directional control valves × 4** ([DESIGN-HYDRAULIC/HYDRAULIC_BOM.md](../DESIGN-HYDRAULIC/HYDRAULIC_BOM.md) §3). The four enum values map to Parker spool-code suffixes in the D1VW00*N*CNKW SKU: ``tandem`` = `008` (canonical, P→T through-pass, A&B blocked, inherent load-hold); ``float`` = `004` (Grainger SKU 2NMU8, pre-BC-18 interim, A&B→T vented); ``closed`` = `001` (all ports blocked); ``open`` = `002` (all ports through to T). | Round 43 / BC-19. Different spool codes are drop-in identical mechanically (same NG06/D03/CETOP-3 mounting, same coils, same DIN-43650 connectors); only the valve-stack SKU number changes. |
| ``hydraulic.load_holding`` | Tier 1 / **Pilot-operated check valves × 2** (arm rod-side + bucket rod-side) for ``po_check``; Tier 1 / **Counterbalance valves** (e.g. Sun CBEH series) for ``counterbalance``; ``spool_inherent`` and ``none`` add no parts. | Round 43 / BC-19. The PO-check pair is the pre-BC-18 interim bill-of-materials documented in [DESIGN-HYDRAULIC/HYDRAULIC_BOM.md](../DESIGN-HYDRAULIC/HYDRAULIC_BOM.md) §3 and survives as the OSE-legacy reference build. ``spool_inherent`` (canonical with tandem) requires no external load-hold hardware. ``counterbalance`` is the high-performance build's choice. ``none`` is permitted only with venting spools and explicit operator acknowledgement that the load will drift. |
