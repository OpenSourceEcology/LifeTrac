...
| Link | Direction | Rate |
|---|---|---|
| Handheld / base -> tractor `ControlFrame` (LoRa) | uplink | Event-driven under one-radio FHSS: immediate on change; held nonzero lease refresh 5-10 Hz; neutral steady refresh 1-2 Hz |
| Active source `HeartbeatFrame` (LoRa) | uplink | 1-2 Hz steady source presence; P0 burst for E-stop, take-control, handover, and link-tune events |
...
If any of these change, update `LORA_PROTOCOL.md`, `TRACTOR_NODE.md`, **and** this table. The 2026-05-27 one-radio FHSS policy supersedes the older "next 50 ms frame is the retry" wording for the mixed image/control bench; the protocol document must be reconciled before firmware implementation.
...
### 8.17 LoRa control-link PHY — **SF7 / BW 250 kHz default, no stale-control retries, event-driven leases under one-radio FHSS, adaptive fallback when SNR margin degrades**

> **2026-05-27 FHSS mixed-mode update:** the old constant-20-Hz control stream is no longer the preferred one-radio FHSS target. The base sends `ControlFrame` state immediately when operator input changes, refreshes held nonzero state as a short lease, sends neutral/source presence much more quietly, and lets the tractor-local watchdog neutralize on silence. This frees reverse windows so the base can remain in RX for tractor image/telemetry traffic.
...
- **Default PHY for control + heartbeat:** SF7 / BW 250 kHz / CR 4/5 for the default rung, matching `DECISIONS.md` D-A2 and `LORA_PROTOCOL.md`; SF8/SF9 fallback rungs may use BW125 for link budget. The old BW125 default remains an archived note, not the current target.
- **No retries on stale `ControlFrame`.** The MAC layer's `transmit()` retry loop is **disabled for P0 ControlFrame**. We only ever care about the *newest* control snapshot. Retrying a stale stick position can only hurt: it consumes channel time the next-newer frame needs, and it can briefly re-apply a stick deflection the operator already released. CSMA's channel-busy backoff still applies (one polite check, no second attempt), but if the channel is busy we drop that stale snapshot and send the freshest state at the next opportunity.
- **Send on change, then lease-refresh.** Operator input changes beyond deadband transmit immediately at the next reverse opportunity. Held nonzero commands refresh at 5-10 Hz with a short lease; held neutral commands and source-presence heartbeats refresh at 1-2 Hz. E-stop, take-control, handover, and link-tune events remain immediate P0 bursts.
- **Layered keepalive cutoffs.** Do not use one global heartbeat timeout for every meaning of liveness. Candidate bench values: nonzero actuator lease expires to neutral in 500-750 ms; neutral/source presence can be more forgiving at 1.5-2.0 s; E-stop latches until explicit clear. The tractor-local failsafe still wins on silence.
- **One-radio FHSS mixed scheduling.** Tractor image/telemetry packets are the hop-clock master while the image path is active. Base control replies ride same-channel reverse windows using a non-advancing reply TX mode, so base control does not seize the FHSS clock. Idle image periods use low-rate tractor master keep-alives only as fast as needed for the source-presence/lease targets.
...
- **Air-time impact at each rung** (for the 44-byte encrypted ControlFrame before KISS expansion, estimated): SF7/BW250 is about 46 ms; SF8/BW125 is about 164 ms; SF9/BW125 is about 288 ms. Continuous 20 Hz duplicate controls do not fit the slower fallback rungs; the event-driven lease model is required under one-radio FHSS.
- **Failsafe behavior is layered.** Nonzero actuator state expires quickly to neutral if the lease is not renewed. Source activity can be more forgiving than the actuator lease, but loss of all valid control/heartbeat traffic still drops to `SOURCE_NONE` and the tractor-local watchdog neutralizes the machine.
...
- **FCC §15.247 compliance — 50-channel FHSS supersession (2026-05-27).** The older 8-channel FHSS narrative is superseded for the v25 image/control radio by `REG_PROFILE_FCC_15_247_FHSS_50CH_BW250`: 50 channels across 902-928 MHz, packet-clocked scheduler, A6a hop-sync header, legal dwell ledger, and RFCO evidence. All image, telemetry, control replies, source heartbeats, and master keep-alives must ride the same 50-channel profile once FHSS is the bench default. `BENCH_ONLY_FIXED_915` remains a diagnostic escape hatch only. Field/legal release still requires archived channel-count, hop-distribution, dwell, occupied-bandwidth, OOB, power-clamp, and profile-lock evidence.
...
### 8.20 Image transmit/receive code path — **active firmware priority (2026-05-15)**

The Kurokesu C2 USB camera was bench-validated end-to-end on the tractor X8 on **2026-05-15** using a static `aarch64` FFmpeg binary pushed over ADB: `/dev/video1` ("`C2: C2 KTM-QGFST`") streamed MJPEG @ `1920×1080` and produced a clean 42 KB JPEG (`yuvj420p`, BT.470BG) on first try, after the §8.19 USB host-controller mitigations from the W2-01 work were in place. With pixel acquisition proven, the W2-02 follow-up is now committed as the next active firmware/software workstream — it owns *every* line of code that turns those camera bytes into something the operator's browser actually displays:

**On tractor (Portenta X8 Linux + H747 M7):** the `firmware/tractor_x8/x8_image_pipeline/` capture → tile-diff → ROI-bias → NanoDet detector → tile-delta WebP encoder → fragment → IPC-to-M7 chain (per the [tractor image pipeline checklist in DESIGN-CONTROLLER/TODO.md Phase 5A/5B](TODO.md#phase-5a--foundation-cpu-only-no-ai-accelerator-required-ships-value-alone) and [TRACTOR_NODE.md § Image pipeline](TRACTOR_NODE.md#image-pipeline-portenta-x8-linux-side)), plus the M7 ring-buffer + LoRa TX scheduler that emits `TileDeltaFrame` (topic `0x25`) at P3 without preempting any P0 ControlFrame. Tractor-side detector + `CMD_PERSON_APPEARED` (opcode `0x60`) ride this path.

**On base (Portenta X8 Linux):** the `base_station/image_pipeline/` reassemble → canvas → bg-cache → recolouriser → super-res → independent YOLO safety detector → state-publisher chain (per [BASE_STATION.md § Image pipeline](BASE_STATION.md#image-pipeline-portenta-x8-linux-side)), plus the WebSocket fan-out to the operator browser, the `link_monitor.py` auto-fallback ladder (`CMD_ENCODE_MODE` opcode `0x63`), and the I-frame-loss recovery path (`CMD_REQ_KEYFRAME` opcode `0x62`).
