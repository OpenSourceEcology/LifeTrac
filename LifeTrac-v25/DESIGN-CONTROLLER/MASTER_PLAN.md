...
> **Implementation review blocker (2026-04-27):** the current shared airtime estimator gives a 44-byte encrypted `ControlFrame` about **92 ms** on air at SF7/BW125/CR4-5, before CSMA/backoff and KISS expansion. That cannot support the documented 20 Hz / 50 ms control cadence or the ≤60 % steady-state airtime budget. Do not field-test the LoRa control path until one of these is changed and bench-verified: control bandwidth, control cadence, encryption/framing overhead, or the one-radio traffic split.
...
- **Default PHY for control + heartbeat:** SF7 / BW 125 kHz / CR 4/5. The earlier estimate of ~30 ms for encrypted `ControlFrame` traffic was wrong for BW125; the implementation estimator reports ~92 ms for the 44-byte encrypted payload. Keep the pin as the current design intent, but treat the cadence/PHY mismatch above as a blocking decision before hardware motion.
- **No retries on `ControlFrame`.** The MAC layer's `transmit()` retry loop is **disabled for P0 ControlFrame**. We only ever care about the *newest* control snapshot. Retrying a stale stick position can only hurt: it consumes channel time the next-newer frame needs, and it can briefly re-apply a stick deflection the operator already released. CSMA's channel-busy backoff still applies (one polite check, no second attempt), but if the channel is busy we drop the frame and rely on the next tick. `TelemetryFrame` retains its existing 3-attempt CSMA behavior; only P0 control changes. The tick rate remains an open blocker until the airtime mismatch is resolved.
...
- **Air-time impact at each rung** (for the 44-byte encrypted ControlFrame before KISS expansion, estimated): SF7 ~92 ms → SF8 ~164 ms → SF9 ~288 ms. These values are incompatible with a 20 Hz control stream on the current one-radio shared channel; resolve before field testing.
