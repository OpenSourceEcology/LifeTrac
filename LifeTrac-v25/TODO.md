...
- [ ] **Coordinated modem-rung switch (D-VS8, `VECTOR_SCENE.md` §4.6):** SF7/8/9 rungs inside each regulatory profile on the L072, switched with a confirmed and scheduled handshake (`0x6D`–`0x70`), per-side revert timers and a rendezvous rung with a beacon; +5 dB (DTS) / +8 dB (FHSS) at the slowest rung, where only the vector mode still fits. Needs a new L072 CFG key, a header nibble, a SIL model and two bench legs; the range-edge attenuator leg would be the campaign's first field-range data.
