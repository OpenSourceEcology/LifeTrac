...
| Encode modes | `EncodeMode` 0–7 on the base at the time of the survey (`lora_proto.py:80-91`; 0–8 since #131); the tractor adds `RAWSTREAM = 8`, which the base rejected (`camera_service.py:501`; `image_rx_daemon.py:1175-1203`) until #131 added it to the enum with a parity test (2026-09-24). `wireframe` (3) is a legacy alias for grey WebP at q ≤ 20; no wireframe or vector code runs. |
| Mode selection | Operator-only: `_ENCODE_MODE_UI_CHOICES` = full, y_only, motion_only, mono_g4 and, since #131, rawstream (`web_ui.py:103-109`), persisted in `.encode_mode_override`, sent as `0xFB 0x63 [mode, quality]`, acked by `0x68` with the codec id, and confirmed by the codec byte of later frames. No automatic encode ladder runs in production (`web_ui.py:91-92`). |
...
- **Airtime is quantised** (3.5 B per 1.28 ms step at BW500): a 12 B on-air frame — a `0xFB` command or a skip frame — costs 10.3 ms, and a VS frame with a 12 B body is 30 B on air (6 B `TileDeltaFrame` + 4 B fragment + 8 B hop headers) and costs 18.0 ms at BW500 (§3.1 of the design).
