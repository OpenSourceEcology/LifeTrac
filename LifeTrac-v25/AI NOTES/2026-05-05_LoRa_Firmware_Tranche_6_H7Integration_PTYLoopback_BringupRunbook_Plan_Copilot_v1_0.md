# LoRa Firmware Tranche 6 — H7 Runtime Integration, PTY Loopback Harness, Bring-up Runbook (Copilot v1.1)

Date: 2026-05-05
Status: REVISED — Gemini §15 + Copilot self-review §16 amendments applied
...
| R5 | `mh_runtime` competes with another peripheral on the chosen HardwareSerial (per §15.4 + §16.10) | Med | High | `LIFETRAC_MH_SERIAL` has **no implicit default** — build fails if undefined. Bench operator MUST schematic-verify Max Carrier UART routing and document the chosen Serial in the bring-up runbook. R5 retired once schematic verification is captured in `bench-evidence/T6_bringup_<date>/`. |
