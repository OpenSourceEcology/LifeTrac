...
Invoke-AdbStep -StepName "push firmware image" -Arguments @("-s", $serial, "push", $image, "$tmp/firmware.bin")
Invoke-AdbStep -StepName "run on-device flash pipeline" -Arguments @("-s", $serial, "shell", "echo fio | sudo -S bash $tmp/full_flash_pipeline.sh $tmp/firmware.bin $serial")