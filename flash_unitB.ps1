...
adb -s $serial push "$image" $tmp/lora_ping_rx_active.bin
adb -s $serial shell "echo fio | sudo -S bash $tmp/full_flash_pipeline.sh $tmp/lora_ping_rx_active.bin $serial"